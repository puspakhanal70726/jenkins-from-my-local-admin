...
resource "aws_autoscaling_policy" "AddInstancesPolicyformemory" {
...
resource "aws_autoscaling_policy" "RemoveInstancesPolicyformemory" {
...
alarm_name = "sagar-memory-alarm1"
alarm_description = "sagar-memory-alarm1"
comparison_operator = "GreaterThanOrEqualToThreshold"
evaluation_periods = "2"
...
actions_enabled = true
alarm_actions = ["${aws_autoscaling_policy.AddInstancesPolicyformemory.arn}"]
}
...
actions_enabled = true
alarm_actions = ["${aws_autoscaling_policy.RemoveInstancesPolicyformemory.arn}"]
}
